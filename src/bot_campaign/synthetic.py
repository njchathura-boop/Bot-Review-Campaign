...
SAMPLE_COUNTS = {
    name: max(10, count // 100) for name, count in DEFAULT_COUNTS.items()
}
PRESETS = {"sample": SAMPLE_COUNTS, "full": DEFAULT_COUNTS}
...
def _scenario_records(
    ...
    for index in range(count):
        ...
        if scenario == "organic":
            ...
            template = templates[group_number % len(templates)]
            text = template.format(
                product=product[0].replace("-", " "),
                good=product[1],
                bad=product[3],
            )
        ...
        yield {
            ...
            "generator_name": "deterministic-template-engine",
...
def generate_dataset(
    ...
    """Generate reproducible, provenance-labelled campaign scenarios."""
    output = Path(output)
    output.parent.mkdir(parents=True, exist_ok=True)
    if preset not in PRESETS:
        raise ValueError(f"Unknown preset {preset!r}; choose from {sorted(PRESETS)}")
    counts = dict(counts or PRESETS[preset])
    ...
    manifest = {
        ...
        "note": "Synthetic labels are controlled scenario labels, not production ground truth.",
    }

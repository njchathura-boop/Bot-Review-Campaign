...
from bot_campaign.features import DEFAULT_PRIOR_MEAN_RATING, NO_PRIOR_REVIEW_HOURS, behavioral_features, text_metadata


def test_behavioral_features_use_only_prior_rows():
    reviews, _ = load_reviews("data/sample/campaign_reviews.jsonl")
    ...
    assert not frame[["user_prior_mean_rating", "product_prior_mean_rating", "hours_since_user_review"]].isna().any().any()
